              ...
              {NAV_LINKS.map((link) => {
                ...
                if (link.hasDropdown) {
                  return (
                    ...
                    >
                      <Link to={link.href} className={`nav-link nav-dropdown-trigger${isActive ? ' active' : ''}`}>
                        {link.text}
                      ...
                      {servicesOpen && (
                        <div className="nav-dropdown">
                          {siteConfig.footer.serviceLinks.map((s) => (
                ...
                return (
                  <Link key={link.href} to={link.href} className={`nav-link${isActive ? ' active' : ''}`}>
                    {link.text}
                  </Link>
                )
              })}
                ...
                {NAV_LINKS.map((link) => {
                  if (link.hasDropdown) {
                    return (
                      <div key={link.href}>
                        <button
                          className="nav-link py-2 border-b border-[var(--color-border)] w-full flex items-center justify-between"
                        ...
                        {mobileServicesOpen && (
                          <div className="pl-4 flex flex-col gap-1 py-2">
                            <Link to={link.href} onClick={() => setMobileOpen(false)}
                              className="nav-link py-1 text-sm text-text-secondary">
                              All Services
                            </Link>
                            {siteConfig.footer.serviceLinks.map((s) => (
...
function ChevronIcon({ open }: { open: boolean }) {
  return (
    <svg
      ...
      style={{ marginLeft: 4, transition: 'transform 0.2s', transform: open ? 'rotate(180deg)' : 'rotate(0deg)', display: 'inline' }}
    >
...
function HamburgerIcon() {
  return <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>
}
function XIcon() {
  return <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
}
